...
import com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.APPLICATION_THEME_OBJECT
import com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.FILE_NAME
...
class SharedPreferences private constructor(context: Context) {
    ...
    fun setCartCountString( value: String) {
        appSharedPreferences!!.edit().putString(com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.SHARED_PREFERENCES_CART_COUNT, value).apply()
    }

    fun getCartCountString(): String? {
        return appSharedPreferences!!.getString(com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.SHARED_PREFERENCES_CART_COUNT, "0")
    }
    ...
    fun setProfile(profile: GetMyProfile.Data) {
        appSharedPreferences!!.edit().putString(
            com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.PROFILE,
            Gson().toJson(profile)
        ).apply()
    }
    ...
    fun setAddToCartData(body: GetCartResponse) {
        appSharedPreferences!!.edit().putString(
            com.plugable.mcommerceapp.crazypetals.utils.constants.SharedPreferences.SHARED_PREFERENCES_CART_DATA,
            Gson().toJson(body)
        ).apply()
    }
